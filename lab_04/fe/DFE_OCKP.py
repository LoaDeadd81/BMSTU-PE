...
from lab_04.fe.PartLinearDFE import PartLinearDFE


class DFE_OCKP(PartLinearDFE):
    ...
    def expand_root_matrix(self):
        combinations = [(x, x) for x in range(self.factor_num)]
        for i in range(self.N_0):
            for j in range(len(combinations)):
                tmp = 1

                for k in range(len(combinations[j])):
                    tmp *= self.matrix[i][combinations[j][k] + 1]

                self.matrix[i].append(tmp)

    def expand_ockp_part_matrix(self):
        for i in range(self.N_0, self.N):
